...
class APIManager: NSObject {
    ...
    func generateAPIRequest<T: Codable>(reqEndpoint: APIConstants, type: T.Type, isAccessToken: Bool = false, reqBodyData: [String:Any]?, completion: @escaping completionDataHandler<T>)  {
        ...
        {
            request.setValue(SessionManager.shared.setLoginUserDataValue?.token ?? "", forHTTPHeaderField: "Authorization")
        }
            ...
            .responseJSON { response in
                ...
                if let httpError = response.error {
                    ...
                    if statusCode == 401 {
                        ...
                        let errorMsg = "Unauthorized! Please login again."
                   
                        
                        let _ = CustomAlertController.alert(title: APP.appName, message: errorMsg, acceptMessage: "Ok") {
    ...
    //MARK: - success Request without Data response
    func generateAPINilDataRequest<T: Codable>(reqEndpoint: APIConstants, type: T.Type, isAccessToken: Bool = false, reqBodyData: [String:Any]?, completion: @escaping completionDataHandler<T>)  {
        ...
        apiRequestToServerWithoutData( apiRequest: request, type: type, completion: completion)
    }
    
    //MARK: - Direct response without success key and all
    private func apiRequestToServerWithoutData<T: Codable>( apiRequest: URLRequest, type: T.Type, completion: @escaping completionDataHandler<T>) {
    ...
    func uploadFiles<T: Codable>(reqEndpoint: APIConstants, type: T.Type, reqBodyData: [String:Any]?, documentArray: [String:[Data]], mediaType : String, isAccessToken: Bool = false, completion: @escaping completionDataHandler<T>)  {
              ...
              for (key,value) in documentArray
              {
                  ...
                  */
                  ...
                  for i in 0..<value.count
                  {
                      if mediaType == "video"
                      {
                          multipartFormData.append(value[i], withName: key , fileName: "\(Int(Date().timeIntervalSince1970)).mp4", mimeType: "video/mp4")
                      }else{
